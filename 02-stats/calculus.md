...
So to recap $A(x)$ which gives the area under the graph of x2 between some fixed-left point and some variable right point, it gives us a clear panorama that many practical problems can be approximated by adding up a large number of small things that can be reframed as a question about the area under a certain curve.
